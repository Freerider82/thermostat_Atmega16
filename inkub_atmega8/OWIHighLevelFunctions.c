    ...
    while (currentBit <= 64)
    {
        ...
        else // Both bits 0
        {
			...
			{								
				...
				//printf(">newDeviation %d\r",newDeviation);
            }
            // If current bit in bit pattern = 0, then this is
            // out new deviation.
			
			
			//» как возникнуть услови€ что currentBit < lastDeviation ?
			//¬озможно при подключении нового датчика
        ...
        {
            bitMask = 0x01;
			//printf("ElemPattern 0x%x",*bitPattern);
            bitPattern++;
        }
		//printf(" bitPattern %d",*bitPattern);
    }
    return newDeviation;
}
...
unsigned char OWI_SearchDevices(OWI_device * devices, unsigned char numDevices, unsigned char pin, unsigned char *num)
{
    ...
    unsigned char numFoundDevices=0;
    ...
    do  
    {
		// опирование 8 байт  из currentID в newID
      memcpy(newID, currentID, 8);
      if (!OWI_DetectPresence(pin)){
		  //printf("no datchik");
        return SEARCH_ERROR;        //¬ернуть ошибку если датчика нет
      };
...
uint8_t  DS18B20_WriteScratchpad(uint8_t bus, uint8_t * id, uint8_t th, uint8_t tl,uint8_t resolution){
	...
	//ѕроверка записалась ли конфигураци€ ?
	OWI_DetectPresence(bus);
	OWI_MatchRom(id, bus);
